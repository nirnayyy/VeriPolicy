...
export function WorldMap({ 
  ...
  const svgMap = useMemo(
    () => map.getSVG({
      radius: 0.22,
      color: theme === "dark" || theme === "forest" ? "#FFFF7F40" : "#00000040",
      shape: "circle",
      backgroundColor: theme === "dark" || theme === "forest" ? "black" : "white",
    }),
    [map, theme]
  );
            ...
            <g key={`points-group-${i}`}>
              <g key={`start-${i}`}>
                <motion.g
                  ...
                  onClick={() => onPointClick?.(dot.start.label || "")}
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center justify-center h-full">
                        <span className={`px-2 py-0.5 rounded-md bg-white/95 dark:bg-black/95 text-black dark:text-white border border-gray-200 dark:border-gray-700 shadow-sm ${labelClassName}`}>
                          {dot.start.label}
                        </span>
                      </div>
                    </foreignObject>
              ...
              <g key={`end-${i}`}>
                <motion.g
                  ...
                  onHoverEnd={() => setHoveredLocation(null)}
                  onClick={() => onPointClick?.(dot.end.label || "")}
                  className="cursor-pointer"
                  whileHover={{ scale: 1.2 }}
                  transition={{ type: "spring", stiffness: 400, damping: 10 }}
                >
