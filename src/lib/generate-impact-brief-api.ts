...
export async function handleGenerateImpactBriefRequest(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (!policyId) {
      return new Response(JSON.stringify({ error: "policyId is required" }), { status: 400, headers: { "content-type": "application/json" } });
    }
    ...
    if (!row) return new Response(JSON.stringify({ error: "policy not found" }), { status: 404, headers: { "content-type": "application/json" } });

    const cachedBrief = parseStoredImpactBrief(row.impact_brief);
    if (cachedBrief) {
      return new Response(JSON.stringify({ policyId, impactBrief: cachedBrief }), { status: 200, headers: { "content-type": "application/json" } });
    }
    ...
    return new Response(JSON.stringify({ policyId, impactBrief: result }), { status: 200, headers: { "content-type": "application/json" } });
  } catch (err: any) {
...
export default { handleGenerateImpactBriefRequest };
